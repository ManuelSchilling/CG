...
public class RenderFisch {
	...
	public static void renderFisch(float x, float y, int off, float winkel, int radius, float r, float g, float b) {
		...
		for (int angle=0; angle<360; angle+=5) {
	       glVertex2f((float)Math.sin(angle) * radius,(float)Math.cos(angle) * radius);
	    }
	    glEnd();
	    ...
	    glBegin(GL_TRIANGLE_FAN);
	    for (int angle=0; angle<360; angle+=5) {
			glVertex2f((float)Math.sin(angle) * (radius-radius/4),radius + (float)Math.cos(angle) * (radius-radius/4));
		}
	    glEnd();
	    ...
	    glBegin(GL_TRIANGLE_FAN);
		for (int angle=0; angle<360; angle+=5) {
			glVertex2f((float)Math.sin(angle) * (radius-radius/3),radius*2 + (float)Math.cos(angle) * (radius-radius/3));
		}
		glEnd();
	...
	public static void renderSwarmObjectWithForces(float x, float y, int radius, Vektor2D velocity, Vektor2D acceleration, float r, float g, float b) {
